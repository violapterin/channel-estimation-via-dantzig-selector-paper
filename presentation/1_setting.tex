...
{
\I Given MIMO channel representation \m {\M {H} \in \NC \MB {C} ^{N_H \D N_H}} with uniform linear array, having \m {L} paths.
...
\I Magnitude constraint \m {{\Nm {\M {F}_R}_{\SB {n_H, n_R}}} = 1}, \m {{\Nm {\M {W}_R}_{\SB {n_R, n_H}}} = 1} 
}
% XXX % % XXX % % XXX % % XXX % % XXX % % XXX %

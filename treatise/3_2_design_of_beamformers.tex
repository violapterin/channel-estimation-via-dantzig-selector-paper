...
Let us make it simple and set each entry of \m {\M {F} _B} to be i.i.d.\ normal random variables with mean 0, standard deviation \m {1/2}, multiplied by a normalizing constant \m {\l_B >0}.
We know that the magnitude \m {\RB {\M {F} _B} _{\SB {n_R, n_B}}} follows Rayleigh distribution.
\m {\RB {\M {W} _B ^\Tr} _{\SB {n_B, n_R}}} is set in an entirely analogous manner.
In addition, let \m {\RB {\M {F} _R} _{\SB {n_H, n_R}}} be uniformly distributed on the unit circle on the complex plane.
\m {\RB {\M {W} _R ^\Tr} _{\SB {n_B, n_R}}} is set in an entirely analogous manner.

It turns out we have results that guarantee that both \m {\M {F} _B ^\Tr} and \m {\M {F} _R ^\Tr} have RIP, and same can be said of \m {\M {W} _B} and \m {\M {W} _R}.
...
However, it is not clear that \m {\M {F} _B ^\Tr \M {F} _R ^\Tr}, the product of two RIP matrices, have RIP, nor that, \m {\M {P}}, the result of Kronecker product, has RIP.
...
Use \m {\chi _{i,j}} to denote independent complex standard normal r.vs.
We know
%
\DispNum {C':C':WB:j2} {
...
%
We want a large deviation result for i.i.d.\ sum of chi-square random variables.
...
Here, we consult Haviv and Regev, \quotation {The restricted isometry property of subsampled Fourier matrices} \cite [KlM17].
We need their Thm.4.5, which guarantees RIP w.r.t.\ \m {\d_s}, and probability lower bound, for DFT submatrix.
...
They use Chernoff-Hoeffding bounds, but write the exponent in the form of \m {\Omega}.

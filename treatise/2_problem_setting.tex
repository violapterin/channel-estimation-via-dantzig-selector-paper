...
We consider the hybrid beamforming at both transmitter and receiver end, as shown in the figure.
That is to say, each end consists of both digital and analog precoders and combiners, as follows.
In the transmitter end, there are (seeing towards receiver end), digital precoder \m {\M {F} _B \in \MB {C} ^{N_R \D N_B}} and analog precoder \m {\M {F} _R \in \MB {C} ^{N_H \D  N_R}}.
Similarly, in the receiver end, there are (seeing towards transmitter end) digital combiner \m {\M {W} _R \in \MB {C} ^{N_R \D N_H}} and analog combiner \m {\M {W} _B \in \MB {C} ^{N_B \D  N_R}}.
...
Since we restrict our discussion to a specific snapshot of time, the noise term may be simply taken as a matrix \m {\M {Z} \in \NC \MB {C} ^{N_B \D N_B}} with each entry being i.i.d.\ Standard Normal.
...
%
At this stage we would like to apply DS on sparse vectors, as motivated in chapter 1.
However, there is no guarantee that \m {\V {h}} is sparse.

\stopsection

\startsection [title={Spacial frequency domain}]

Introduce the discrete Fourier transform matrix \m {\M {K}}
%
\DispNum {K:K:CN:H1} {
...
%
Sans serif \m {\Ss {e}} and \m {\Ss {i}} denote the natural base and imaginary unit, respectively.
...
%
it can be interpreted as spatial frequency domain representation of \m {\M {H}}.
Then
%
\DispNum {Y:Y:WB:NY} {
\NC \M {Y}
...
%
If it turns out that \m {\M {G}} is sparse, then we may recover \m {\M {G}} instead, justifying the use of DS.
...
%
\item Find \m {\M {P} \in \MB {C} ^{N_B^2 \D N_H^2}}, \m {\V {y} \in \MB {C} ^{N_H^2}} as in above.
%

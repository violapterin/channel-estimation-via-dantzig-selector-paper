...
Indeed, the estimation of MIMO channel amounts to determining all parameters of the channel representation.
Our task amounts to invert a linear system whose dimension is the number of antennae.
...
Candès and Tao showed that, in the noiseless case, a \m {\ell_1}-minimization program recovers the \m {N_m}-dimensional signal, with fewer than \m {N_p} measurements, under overwhelming probability.
...
we must make a series of carefully chosen but random measurements.
...
At the same time, Tropp and Gilbert \cite [TrG07b] suggested likewise that a greedy algorithm called orthogonal matching pursuit (OMP) may reconstruct a sparse signal sufficiently well in an overwhelmingly probability. 
In the situation of \Rf {y:y:Qx:xz}, we would pick up the columns of \m {\M {Q}} greedily, hoping to correspond to the nonzero components of \m {\V {x}}.
...
In this treatise, we illustrate that DS may outperform OMP and other methods.
...
Numerical results show that in our configuration, where we have uniformly random angles of departure and arrival, DS is superior, but is achieved at the cost of higher complexity.

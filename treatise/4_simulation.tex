...
In the following, we plot the reciprocal of noise level vs relative error norm, for different values and ratios of \m {N_H} and \m {N_B}.

\blank [big]
\externalfigure [assorted-square-small-error.png] [wfactor=fit, hfactor=fit]
\FigureCaption {Assorted methods, small, \m {N_B = 12, N_H = 12}, error norm.}
\blank [big]
%
\blank [big]
\externalfigure [assorted-square-medium-error.png] [wfactor=fit, hfactor=fit]
\FigureCaption {Assorted methods, medium, \m {N_B = 18, N_H = 18}, error norm.}
\blank [big]
%
\blank [big]
\externalfigure [assorted-square-big-error.png] [wfactor=fit, hfactor=fit]
\FigureCaption {Assorted methods, big, \m {N_B = 24, N_H = 24}, error norm.}
\blank [big]
%
\blank [big]
\externalfigure [assorted-narrow-small-error.png] [wfactor=fit, hfactor=fit]
\FigureCaption {Assorted methods, small, \m {N_B = 10, N_H = 12}, error norm.}
\blank [big]
%
\blank [big]
\externalfigure [assorted-narrow-medium-error.png] [wfactor=fit, hfactor=fit]
\FigureCaption {Assorted methods, medium, \m {N_B = 15, N_H = 18}, error norm.}
\blank [big]
%
\blank [big]
\externalfigure [assorted-narrow-big-error.png] [wfactor=fit, hfactor=fit]
\FigureCaption {Assorted methods, big, \m {N_B = 20, N_H = 24}, error norm.}
%
\blank [big]
\externalfigure [assorted-wide-small-error.png] [wfactor=fit, hfactor=fit]
\FigureCaption {Assorted methods, small, \m {N_B = 8, N_H = 12}, error norm.}
%
\blank [big]
\externalfigure [assorted-wide-medium-error.png] [wfactor=fit, hfactor=fit]
\FigureCaption {Assorted methods, medium, \m {N_B = 12, N_H = 18}, error norm.}
%
\blank [big]
\externalfigure [assorted-wide-big-error.png] [wfactor=fit, hfactor=fit]
\FigureCaption {Assorted methods, big, \m {N_B = 16, N_H = 24}, error norm.}
...
We wonder why OMP is so bad.
A crucial reason might be that in Lee, Gil, and Lee \cite [LGL16], they proposed a high-complexity convex program for the beamformers, which they gave approximation, and we have here a low-complexity, randomly-generated beamformers.
...
It remains to see how OMP and DS compare, if done exactly under their settings.

The trend between \m {\s} (dimensionless noise level) and \m {\T {\chi}} (simulated error) is not clear, especially for OMP and lasso.
...
Varying thresholds of DS (namely \m {\g_{\Rm {DS}}}) and OMP (namely \m {\h_{\Rm {OMP}}}) appears not to affect much.
It may have been that the iteration stabilizes much earlier than is constrained by maximum iteration number.
While we choose \m {\g_{\Rm {DS}}} to be in the order of magnitude of \m {\s}, it seems, as signal level is low, a greater value may be better.

Unfortunately, we report that CVXPY sometimes gives overflowing outputs, some of them as large as \m {10^{11}}.
...
We are not sure either why \m {\T {\chi}} is an overestimation of \m {\chi} by several orders of magnitude.
A possible explanation is that the non-sparsity of \m {\V {g}} undermines the analysis in chapter 3, despite our attempts to account for that effect, which remain to be investigated in the future.

We end this section by showing two cases where DS works or fails, in figures 21 and 22.

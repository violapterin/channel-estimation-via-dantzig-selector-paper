...
\Disp {
\NC H
=\sum_{l=1}^L \alpha_l \V {a} \SB { \F {d} {\lambda} \sin \f_l' } \V {a} \SB { \F {d_{\RM {arr}}} {\lambda} \sin \th_l' }^\H \NR
}
%
...
\Disp {
\NC H
=\sum_{l=0}^L \alpha_l \V {a} \SB {\f_l} \V {a} \SB {\th_l}^\H
...
%
\Disp {
\NC \Nm {\M {F}_R \SB {n_h, n_r}} = \NC 1, \NR[+]
\NC \Nm {\M {W}_R \SB {n_r, n_h}} = \NC 1, \NR[+]
...
%
\Disp {
\NC Y \SB {\omega_h, \omega_e}
:= \NC W_B \SB {\omega_r} W_R \SB {\omega_r} \SB {H \SB {\omega_h} F_R \SB {\omega_t} F_B \SB {\omega_t} +Z \SB {\omega_z} } \NR
}
%
...
\NC \M {K} \SB {n_1, n_2} = \NC \F {1} {\R {N_H}} \RM {e}^{2\pi i n_1 n_2 /N_H}, \NR[+]
...
\Disp {
\NC \M {Y}
:=\M {W}_B \M {W}_R \M {K} \SB { \M {G} \M {K}^\H \M {F}_R \M {F}_B +\M {K}^\H \M {Z} }
...
\NC \V {g}
:= \NC \RM {vec} \SB {\M {G}}
\in \MB {M}_{\MB {C}} \SB {N_g} \NR[+]
\NC \V {z}
:= \NC \RM {vec} \SB {\M {K}^\H \M {Z}}
\in \MB {V}_{\MB {C}} \SB {N_y} \NR
}
...
\RM {subject} \; \RM {to} \quad  \MC \VNm {\M {P}^\H \RB {\V {y} -\M {P} \V {g}'}} _\infty \leq \gamma \NR
\stopcases \NR
}
%

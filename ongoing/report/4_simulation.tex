...
As of now, we have transformed the sparse-recovery problem of complex matrix \m {H} to complex vector \m {h}, and finally to real vector \m {\MC {R} (h)}.
...
{Definition}
{
For \m {x \in \MB {V} (M)}, we say \m {\MC {R} (x)} is the real representation of \m {x}, defined by
%
\Disp {
\NC \MC {R} (x) \in \NC \MB {V} (2M) \NR[+]
\NC \MC {R} (x) \DB {m} =
\NC \startcases
\MF {Re} (\MC {R} (x)\DB {m}), \quad \MC m =2m' \NR
\MF {Im} (\MC {R} (x)\DB {m}), \quad \MC m =2m'+1 \NR
\stopcases \NR[+]
\NC m' \NC =  1, \dots, M \NR
}
%
...
{Definition}
{
For \m {A \in \MB {M}_{\MB {C}} (M_1, M_2)}, we say \m {\MC {R} (A)} is the real representation of \m {A}, defined by
%
\Disp {
\NC A \in \NC \MB {V}_{\MB {R}} (2M_1, 2M_2) \NR[+]
\NC A (m_1,m_2) =
\NC \startcases
\MF {Re} (A (m_1,m_2)), \quad \MC m_1 =2m_1'-1,\; m_2 =2m_2'-1  \NR
\MF {Im} (A (m_1,m_2)), \quad \MC m_1 =2m_1',\; m_2 =2m_2'-1  \NR
-\MF {Im} (A (m_1,m_2)), \quad \MC m_1 =2m_1'-1,\; m_2 =2m_2'  \NR
\MF {Re} (A (m_1,m_2)), \quad \MC m_1 =2m_1',\; m_2 =2m_2'  \NR
\stopcases \NR[+]
...
\NC \M {\T {P}}
\NC = \MC {R} (\M {P})
\in \MB {M}_{\MB {C}} (2 N_y^2, 2 N_h^2) \NR
}
%
such that
%
...
%
\Disp {
\NC \M {I}_{n_y}^{(y)} \in \NC \MB {M}_{\MB {R}} (2N_y^2, 2N_y^2) \NR[+]
\NC \M {I}_{n_y}^{(y)} (n_y')
\NC =
\startcases
...
0, \quad \NC otherwise
\stopcases \NR[+]
\NC \M {I}_{n_h}^{(h)} \in \NC \MB {M}_{\MB {R}} (2N_h^2, 2N_h^2) \NR[+]
\NC \M {I}_{n_h}^{(h)} (n_h')
\NC =
\startcases

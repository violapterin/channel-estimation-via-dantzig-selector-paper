...
The real and imaginary part of \m{a} will be denoted respectively as \m{\MF{Re} (a)} and \m{\MF{Im} (a)}, respectively, thus \m{a =\MF{Re} (a) + i \MF{Im} (a)}
...
Particularly, define \m{\V{1} =\IP {1, \dots, 1}} and \m{\V{1} =\IP {1, \dots, 1}}.
...
Not to avoid confusion, we call the Hilbert space \m{\MB{K}^N} over \m{\MB{K}} to be \m{\MB{V}_{\MB{K}} (N)}.
We also call the collection of \m{M} by \m{N} matrices to be \m{\MB{M}_{\MB{K}} (M,N)}, and its role as linear transformation is understood.
Inner product of \m{\V{a}} and \m{\V{b}} is denoted as \m{\IP{ \V{a}, \V{b} }}.
Slightly abusing notation, we denote as \m{\V{a} (n)} the \m{n}-th component of \m{\V{a}}, and denote as \m{\V{A} (m,n)} the \m{(m,n)}-entry of \m{\V{A}}
Particularly, \m{\V{u}_n} is the \m{n}-th unit vector in the Cartesian coordinates (not to be confused with the noise).
\m{\VNm {\V{a}} _p} denotes the \m{\ell_p}-norm of vector \m{\V{a}}.
Recall that \m{\ell_\infty}-norm is the maximum-magnitude norm.

Denote as \m{\MB{P} (X)} the probability, and \m{\MB{E} (X)} the expectation.
...
In vague terms, the capacity \m{C} of a MIMO channel \m{\M{H}} at perfect channel state information is well-known to be roughly in the form \m{\log \det (I +\RM{SNR})}, but that knowledge is hardly always available.
...
Indeed, analysis of precoding algorithm, for example, usually makes use of the sum rate \m{\log \det (I +\RM{SNR})}.
...
We will use \m{T \RB{N_m} =\{1, \dots, N_m\}} to denote the support of \m{\MB{V}_{\MB{K}} (N_m)}.

\Result
{Definition}
{
Suppose \m{\M{A} \in \MB{M}_{\MB{K}} (N_m, N_p)}.
...
{Definition}
{
\m{x \in \MB{V}_{\MB{K}} (N_m)} is called \m{s}-sparse with support \m{\MC{A}}, \m{s <N_m}, if there is \m{\MC{A} \subset T \RB{N_m}} such that
%
\Disp{ 
\NC x_{T(N) -\MC{A}}
=\NC \V{0} \NR
...
{Program}
{
Let \m{P \in \MB{M}_\MB{K} (N_m, N_p)} and \m{y \in \MB{V}_\MB{K} (N_p)} be given.
Find \m{h \in \MB{V}_\MB{K} (N_p)} with
%
\Disp{
\NC
h
...
Now let \m{y =P h}.
Suppose \m{h} is \m{s}-sparse, then the program () completely recovers \m{h}.
That is, a noiseless linear transformation of a sparse vector is completely recovered by a \m{\ell_1}-minimization program with an overwhelming probability.

\Result
{Definition}
{
Consider \m{P \in \MB{M}_{\MB{K}} (N_m, N_p)}, with unity-\m{\ell_2}-norm columns.
For \m{s =1, \dots N_p}, we say that \m{P} satisfies the restricted isometry property (RIP) of sparsity \m{s} with respect to \m{0 \leq \d_s \leq 1}, if, for all \m{s}-sparse \m{\V{x} \in \MB{V}_{\MB{K}} (N_p)}, for all \m{\MC{T} \subset T(N_m)} with \m{\#\{\MC{T}\} \leq s},
%
\Disp{
\NC \NC (1-\d_s) \VNm {V{x}}^2 \NR
\NC \leq \NC \VNm {P_{\MC{T}} \V{x}} _2^2 \NR
\NC \leq \NC (1+\d_s) \VNm {\V{x}} _2^2 \NR[+]
}
}
...
{Program}
{
Let \m{\M{P} \in \MB{V}_\MB{R} (N_m, N_p)} and \m{\V{y} \in \MB{V}_\MB{R} (N_m)} be given.
Find \m{\V{h} \in \MB{V}_\MB{R} (N_m)} by
%
...
\NC \RM{subject}\; \RM{to} \quad \MC \VNm {\M{P}^\H (\M{P} \V{h} -\V{y})} _\infty \leq \gamma \NR
\stopcases \NR
}
}
...
{Theorem}
{
Let each entry of random matrix \m{P \in \MB{M}_\MB{K} (N_m, N_p)} be defined as i.i.d.\ Bernoulli with value \m{\pm 1/\R{N_m}} (that is, Rademacher distribution).
Then, with \m{0 <\epsilon <1},
%
\Disp{
\NC \NC \MB{P} ( \Nm {\VNm {P h} _2^2 -\VNm {h}_2^2} \geq \epsilon \VNm{h} _2^2) \NR
\NC \leq \NC 2 \exp (-n (\epsilon^2/4 -\epsilon^3/6) ) \NR
}
}
...
To explain the idea, consider a linear time-invariant channel that \m{y(t) =(x \star h)(t) + z(t)}, where \m{h(t)} is the channel's impulse response, \m{y(t)} is the output, and, for each time index, \m{x(t)} is i.i.d.\ Rademacher, and \m{z(t)} is i.i.d.\ Gaussian.
...
Same as before, consider \m{\V{y} \in \MB{V}_\MB{K} (N_p)}, \m{\V{h} \in \MB{V}_\MB{K} (N_s)}, where \m{\V{h}} is \m{s}-sparse, with \m{N_p \gg N_s}.
If
\Disp{
\NC \V{y} =\M{P} \V{h}, \NR
}
...
{Definition}
{
Define \m{\RM{vec} (\M{A}) \in \MB{K}^{NM}} to be the vectorization of \m{\M{A} \in \MB{M}_{\MB{K}} (N,M)}.
}

\Result
{Definition.}
{
For \m{\RM{vec} (A) \in \MB{M}_{\MB{K}} (N,M)} and \m{\RM{vec} (A) \in \MB{M}_{\MB{K}} (N,M)}
Define the Kronecker product \m{\RM{vec} (A) \in \MB{K}^{nm}} 
}
...
\Disp{
\NC \RM{vec} (Y)
=\big[ (F^\H) \otimes W \big] \RM{vec} (\M{H}) \NR
}
%

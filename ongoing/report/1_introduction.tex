...
\NC \Mr {subject} \; \Mr {to} \quad \MC \VNm {\M {P}^\Adj \RB {\M {P} \V {h} -\V {y}}} _\infty \leq \gamma \NR
\stopcases \NR
}
\item Output \m {\V {h}}.
}
...
{Definition}
{
Define \m {\Mr {vec} \SB {\M {A}} \in \Mb {K}^{NM}} to be the vectorization of \m {\M {A} \in \Mb {M}_{\Mb {K}} \SB {N,M}}.
}

\Result
{Definition.}
{
For \m {\Mr {vec} \SB {A} \in \Mb {M}_{\Mb {K}} \SB {N,M}} and \m {\Mr {vec} \SB {A} \in \Mb {M}_{\Mb {K}} \SB {N,M}}
Define the Kronecker product \m {\Mr {vec} \SB {A} \in \Mb {K}^{nm}} 
}
...
\Disp {
\NC \Mr {vec} \SB {Y}
=\SB {\RB {F^\Adj} \otimes W} \Mr {vec} \RB {\M {H}} \NR
}
%

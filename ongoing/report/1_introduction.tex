...
We will use \m {T \RB {N_m} =\{1, \dots, N_m\}} to denote the support of \m {\MB {V}_{\MB {K}} \SB {N_m}}.

\Result
{Definition}
{
Suppose \m {\M {A} \in \MB {M}_{\MB {K}} \SB {N_m, N_p}}.
...
{Program}
{
Let \m {P \in \MB {M}_\MB {K} \SB {N_m, N_p}} and \m {y \in \MB {V}_\MB {K} \SB {N_p}} be given.
Find \m {h \in \MB {V}_\MB {K} \SB {N_p}} with
%
\Disp {
\NC
h
...
{Program}
{
Let \m {\M {P} \in \MB {V}_\MB {R} \SB {N_m, N_p}} and \m {\V {y} \in \MB {V}_\MB {R} \SB {N_m}} be given.
Find \m {\V {h} \in \MB {V}_\MB {R} \SB {N_m}} by
%
...
\NC \RM {subject} \; \RM {to} \quad \MC \VNm {\M {P}^\H \RB {\M {P} \V {h} -\V {y}}} _\infty \leq \gamma \NR
\stopcases \NR
}
}
...
%
\Disp {
\NC \NC \MB {P} \SB { \Nm {\VNm {P h} _2^2 -\VNm {h}_2^2} \geq \epsilon \VNm {h} _2^2} \NR
\NC \leq \NC 2 \exp \SB {-n \RB {\epsilon^2/4 -\epsilon^3/6} } \NR
}
}
...
{Definition}
{
Define \m {\RM {vec} \SB {\M {A}} \in \MB {K}^{NM}} to be the vectorization of \m {\M {A} \in \MB {M}_{\MB {K}} \SB {N,M}}.
}

\Result
{Definition.}
{
For \m {\RM {vec} \SB {A} \in \MB {M}_{\MB {K}} \SB {N,M}} and \m {\RM {vec} \SB {A} \in \MB {M}_{\MB {K}} \SB {N,M}}
Define the Kronecker product \m {\RM {vec} \SB {A} \in \MB {K}^{nm}} 
}
...
\Disp {
\NC \RM {vec} \SB {Y}
=\big[ \RB {F^\H} \otimes W \big] \RM {vec} \RB {\M {H}} \NR
}
%

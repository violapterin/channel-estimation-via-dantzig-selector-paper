...
We first establish a technical lemma that shows \m{\RM{supp}(\V{g})} has roughly cardinal \m{L}, which guarantees, at the heart of the argument, that \m{\V{g}} is almost-sparse 
...
\item \m{\MC{B}} be the largest \m{s} position of \m{d_{T(N_g) -\MC{A}}}.
...
Let \m{S:\; \MB{V}_{\MB{K}}(N) \mapsto \MB{V}_{\MB{K}}(N)}, \m{N \in \MB{N}}, be the sorting function for \m{x \in \MB{V}_{\MB{K}}(N))}, so that
\Disp{
...
For \m{x \in \MB{V}_{\MB{K}}(N)}, we say \m{x} is confined in sorted magnitude according to \m{f}, if
\Disp{
\NC \Nm {S(x) \DB{ n } } < \NC f (n),
\quad n \in T\RB{N}. \NR
}
}
...
{Definition}
{
For \m{\V{x} \in \MB{V}_{\MB{K}}(N)}, we say \m{\V{x}} is almost-\m{s}-sparse with \m{\ell_1}-residue \m{R}, if
\Disp{
\NC \sum_{n=s+1}^N \Nm {S(\V{x}) (n)}
\leq \NC R. \NR
}
}
...
Let \m{\f} be given, and array response \m{\V{a} (\f)} defined as in ().
Then \m{\V{a} (\f)} is almost-\m{L}-sparse with \m{\ell_1}-residue \m{R(N_H, L)} to be
%
\Disp{
\NC R
...
\NC = \NC \F{\Nm {\sin (N_H \f'/2)}}{\Nm {\sin (\f' /2)}}, \NR[+]
...
%
\Disp{
\NC \sin x = \NC x -\F{x^3}{6} + \MC{O}(x^5), \quad x \ll 1 \NR
}
%
...
\Disp{
\NC R(\eta) -\F{2\pi} {N_H}
\leq \NC \F{1}{N_H} N_H \int_{\f'=2\pi L/N_H}^{2\pi} \Nm {D(\f')} d \f' \NR
}
Bound \m{D} by above technique, meanwhile we notice \m{\f' <24}.
\Disp{
\NC R(\f)
\leq \NC \F{1}{2\pi} \int_{\f'=2\pi L/N_H}^{2\pi} \F{48}{(24 -\f'^2) \f'} d \f'
+\F{2\pi} {N_H} \NR
...
Then \m{\V{g}} is almost-\m{L}-sparse with \m{\ell_1}-residue \m{R(N_H, L)} to be
%
\Disp{
\NC R
...
\leq \NC \VNm {\V{d}_{\MC{A}}} _1 +2\VNm {\V{g}_{C}} _1 \NR
}
}
...
This only depends on the fact that \m{\M{P}(:, n_g)} has unity \m{\ell_2}-norm, and on the randomness of \m{\V{z}}.
...
%
\Disp{
\NC \IP{ \V{z} -(\V{y} -\M{P} \hat{\V{g}}), \M{P} (:,n_y) }
= \NC \IP{ \M{P} \hat{\V{g}} -\M{P} \V{g}, \M{P} (:,n_y) } \NR
\NC = \NC \IP{ \M{P} \V{d}, \M{P} (:,n_y) } \NR
}

By construction
\Disp{
\NC \VNm { \M{P} (:,n_y)^\H (\V{y} -\M{P} \hat{\V{g}}) } 
\leq \NC \VNm { \M{P}^\H (\V{y} -\M{P} \hat{\V{g}}) } _\infty \NR
\NC \leq \NC 2\R{\log N_g} \NR
}
By triangle inequality, together with Lemma (), we have
\Disp{
\NC \VNm {\M{P} (:,n_y)^\H \M{P} \V{d}} 
\leq \NC \VNm {\IP{ \V{z}, \M{P} (:,n_y) }}  +\IP{ \V{y} -\M{P} \hat{\V{g}}, \M{P} (:,n_y) } \NR
...
\leq \NC \F{1}{1-\d_{2s}} \VNm {P_{\MC{A}\MC{B}}^\Tr P d} _2 +\F{\d_{3s}}{(1-\d_{2s}) \R{s}} \VNm {d_{C}} _1 \NR
}
%
}
...
\Disp{
\NC \VNm {\V{d}} _2
\leq \NC \F{4}{\pi^4} \d_{3L}^4 (1-2\d_{2L}) L^4 ( \log N_H )^4 \NR
}
with overwhelming probability.
}
...
\NC \leq \NC  \RB{ \F{\d_{3L}^2} {L^2 (1-\d_{2L})^2} +\F{1}{L} } \VNm {\V{d}_{\MC{C}}} _1^2
+\F{8 \d_{3L}} {L (1-\d_{2L})^2} \R{N_g \log N_g} \VNm {\V{d}_{\MC{C}}} _1 \NR
\NC \NC \quad +\F{16}{(1-\d_{2L})^2} N_g \log N_g \NR
}

...
Fortunately, they have briefly pointed out, complex DS can be recast as an linear program (LP),
one of the basic convex programming problems for which there are already numerous efficient algorithms.
Moreover, we show that this LP can be recast to another form with the total dimension of variables reduced.

Before that, we again introduce some notation.
We want to represent complex vector and matrices by real vectors and matrices, as follows.
\Result
{Definition}
{
...
The first idea is that \m {\V {g}} can be split into positive and negative parts, making the \m {\ell}-1 norm target function into a inner product, and this already makes Algorithm () a LP, just as noted by Tao [].
The second idea is that, by Real Spectral Theorem, \m {\M {P} ^\Tr \M {P}} can be diagonized, and with suitable choice of variables, we may greatly reduce the overhead due to Newton step.
This has not been remarked on the literature as far as we know.


As pointed out above, let
\Disp {
\NC \M {P} ^\Tr \M {P}
...
can be written as
\Disp {
\NC \M {P} ^\Tr \RB {\M {P} \V {g} - \V {y}} - \g \V {1} + \tp
= \NC \V {0} \NR
\NC \M {P} ^\Tr \RB {\M {P} \V {g} - \V {y}} + \g \V {1} - \tm
= \NC \V {0} \NR
}
for some \m {\tp, \tm} with
\Disp {
...
\Disp {
\NC \kp
= \NC \M {Q} ^\Tr \RB {\M {P} ^\Tr \V {y} + \g \V {1}} \NR
\NC \km
= \NC \M {Q} ^\Tr \RB {\M {P} ^\Tr \V {y} - \g \V {1}} \NR
}
and that, similarly,
...
\Disp {
\NC \VNm {\V {g}} _1
=\NC \IP {\V {1}, \gp} - \IP {\V {1}, \gm} \NR
%
\NC =\NC \IP {\M {Q} ^\Tr \V {1}, \fp} - \IP {\M {Q} ^\Tr \V {1}, \fm} \NR
%
\NC =\NC \IP {\M {Q} ^\Tr \V {1}, \fpU} - \IP {\M {Q} ^\Tr \V {1}, \fmU}
+ \IP {\M {Q} ^\Tr \V {1}, \fpV} - \IP {\M {Q} ^\Tr \V {1}, \fmV} \NR
}

This way, the matrix whose Hessian is required by Newton step would be much more sparse (Boyd p.565)
...
%
\NC \min
\MC \IP {\M {Q} ^\Tr \V {1}, \fpU}
- \IP {\M {Q} ^\Tr \V {1}, \fmU}
+ \IP {\M {Q} ^\Tr \V {1}, \fpV}
- \IP {\M {Q} ^\Tr \V {1}, \fmV} \NR
%
\NC \Rm {subject} \; \Rm {to} \Q
\MC -\QU \fpU -\QV \fpV \preceq 0 \NR
%
\NC \MC -\QU \fmU -\QV \fmV \preceq 0 \NR

\startsection [title={Confirming Restricted Isometry of Beamformer (Deprecated)}]
...
Indeed, \m {\d_s \SB {\M {F} _B} } may be found, along similar lines with Achlioptas (2001) and Baraniuk et.\ al.\ (2008).
We prove a concentration inequality, as explained below, and invoke Chebyshev inequality.
But before that, we have to investigate the moments of each entry of \m {\M {F} _B } (resp.\ \m {\M {W} _B }) and that of \m {\M {F} _R } (resp.\ \m {\M {W} _R }).
...
\DispN {
\NC M_{B,k}
:=\NC \MB {E} \SB {\Nm {\M {F} _B \DB {n_R, n_Y} }^k} \NR
...
\DispN {
\NC M_{R,k}
:=\NC \MB {E} \SB {\Nm {\M {F} _R \DB {n_H, n_R} }^k} \NR
\NC k =\NC 1, 2, 3, \dots \NR
}
...
{Lemma}
{
For any instance of \m {\M {P} } and \m {\M {Q} },
\DispN {
\NC \VNm {\M {P} \V {u}} _2
...
Then, for any fixed \m {\V {u} \in \MB {C} ^{N_Y}}, and for any \m {\e >0},
\DispN {
\NC \MB {P}
\SB {

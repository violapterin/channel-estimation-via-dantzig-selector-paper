...
We may applied DS more than once to better estimate the nonzero components, which let us call the successive estimation method.
We apply DS for the first time.
Then we extract largest components of the estimated vector, and apply DS again.
Then we extract largest components of the second estimated vector, and apply Moore–Penrose inverse.
Finally, the resulting indices set has to be scrambled back corresponding to the original ones.
The idea is mentioned \cite [CaT07] without further numerical verification.
...
If more than one stage of estimation is possible, that is, there are more than one data set, the second step may be run on the second dataset, and so on.
It is not impossible to run it for three or more times, but we are not sure that is helpful.

From our observation, it is not obvious whether the successive estimation method is more accurate.
It might have something to do with appropriately scaling \m {\g_{\Rm {DS}}} with respect to \m {N_H}, and it is not obvious what the best choice is.
We do not use this method in the plots included here.
...
However, there is not simple way to determine the value of \m {\l}.
We try \m {\l = N_H^2 / \g_{\Rm {DS}}} in the simulation.
...
The first series for \m {N_{B,t} = N_H/3} and \m {N_{B,r} = N_H/4}.
The second series for \m {N_{B,t} = N_H/4} and \m {N_{B,r} = N_H/3}.
The third series for \m {N_{B,t} = N_H/3} and \m {N_{B,r} = N_H/3}.
...
The wavelength of carrier \m {\l _{\Rm {ant}}} is set to be \m {0.1}, and the antenna spacing \m {d _{\Rm {ant}}} is \m {0.2}; only their ratio matters.


Denote the threshold for DS to be \m {\g_{\Rm {DS}}}, and similar threshold of Lasso to be \m {\g_{\Rm {Lasso}}}.
In the plots of assorted methods, we set \m {\g_{\Rm {DS}} = 2 \R {\log N_H}} as suggested in \cite [CaT07].
...
We simply take \m {\h_{\Rm {OMP}} = 2 \R {\log N_H}} for \m {\infty}-norm constraint, \m {\h_{\Rm {OMP}} = \R {3 \R {N_{B,r} N_{B,t}}}} for 2-norm constraint.
...
We do not know either whether \m {\T {\chi}} is a good indicator of channel capacity; it seems to be, but actually we did not consider a definite a channel model in this treatise.
...
In Lee, Gil, and Lee \cite [LGL16], a high-complexity nonconvex program is used to design the beamformers, while we have here a low-complexity, random generation of beamformers.
...
In general, complexity of convex programs is difficult to analyze, in that it is not easily to determine how many steps were calculated.
...
Alternatively, suppose Newton method is used, and we have self-concordance for linear program.
...
Lasso is equivalent to a QCQP, and there is not always a closed form for its complexity.
...
Simulation illustrates that \m {t_{\Rm {DS}}} grows tremendously with (in our case) \m {N_H}, agreeing the criticism by Friedlander and Saunders \cite [FrS07] on the high complexity of DS.

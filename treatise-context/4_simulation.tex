...
It might have something to do with appropriately scaling \m {\g_{\Rm {DS}}} with respect to \m {N_H}, and we do not know what the best choice is.
...
\startsubsection [title={Plots of time}]

Regarding runtime statistics, we only plot the case where \m {N_{B,t} = N_{B,r} = N_H /2} with \m {2} stages.

\blank [big]
\externalfigure [small-square-two-time.png] [wfactor=fit, hfactor=fit]
\FigureCaption {\m {N_{B,t} = 3, N_{B,r} = 3, N_H = 6}, error.}
\blank [big]


\blank [big]
\externalfigure [medium-square-two-time.png] [wfactor=fit, hfactor=fit]
\FigureCaption {\m {N_{B,t} = 6, N_{B,r} = 6, N_H = 12}, error.}
\blank [big]


\blank [big]
\externalfigure [big-square-four-time.png] [wfactor=fit, hfactor=fit]
...
In most cases, the estimation fails, and we have \m {\T {\chi}} close to the unity.
Varying thresholds of OMP, namely \m {\h_{\Rm {OMP}}}, does not give different result.
...
The performance of Lasso also fails mostly, and \m {\T {\chi}} even increase as \m {\s} decreases, which is strange.

For Moore Penrose pseudoinverse, \m {\T {\chi}} decreases steadily as \m {\s} decreases.

For DS itself, \m {\T {\chi}} decreases as \m {\s} increases, which is expected, but \m {\T {\chi}} does not go down indefinitely, even for very low \m {\s}.
The values of threshold of DS \m {\g_{\Rm {DS}}} are chosen within the order of magnitude of \m {\s}.
For high signal level, \m {\g_{\Rm {DS}}} does not affect much, but for low signal level a greater value may be more effective.

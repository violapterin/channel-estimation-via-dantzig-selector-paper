...
{
\startitemize[n]
\item Input \m{\M {P} \in \MB {C} ^{N_{B,r} N_{B,t} \D N_{H,t} N_{H,r}}}, \m{\V {y} \in \MB {C} ^{N_{B,r} N_{B,t}}}, \m {\g_{\Rm {DS}} > 0}.
%
...
\NC N_2
=\NC \lfloor 4 \log \log N_{H,t} + \log N_{H,r} \rfloor, \NR
\NC N_1
...
We take \m {N_Y} to be \m {4, 6, 8}, respectively.
...
With low noise, \m {\T {\chi} _{\Rm {DS}} \approx \T {\chi} _{\Rm {OMP}} \approx \T {\chi} _{\Rm {Lasso}} \approx \T {\chi} _{\Rm {LS}}}.
But with high noise, \m {\T {\chi} _{\Rm {OMP}}, \T {\chi} _{\Rm {Lasso}}, \T {\chi} _{\Rm {LS}}} blow up quickly,
and it is usually true that \m {\T {\chi} _{\Rm {DS}} \leq \T {\chi} _{\Rm {OMP}} \leq \T {\chi} _{\Rm {Lasso}} \leq \T {\chi} _{\Rm {LS}}}.
...
For DS itself, first, \m {\T {\chi} _{\Rm {DS}}} certainly decreases as \m {\s} increases, but it does not decrease indefinitely.
Second, \m {\T {\chi} _{\Rm {DS}}} decreases as \m {N_H} increases.
Third, when \m {N_{B,t} /N_{B,r}} is smaller, \m {\T {\chi} _{\Rm {DS}}} is also smaller, because we have a higher sample rate.
...
For our case, that would be \m {\MS {O} \SB {4 N_{H,t} ^2 N_{H,r} ^2 \D 8 N_{H,t} N_{H,r}} = \MS {O} \SB {N_{H,t} ^3 N_{H,r} ^3}}.
...
Since the dimension of \m {\M {P} ^\Adj \M {P}} is \m {N_{H,t} N_{H,r}}, the cost of inversion may be roughly considered to be \m {\MS {O} \SB {N_{H,t} ^2 N_{H,r} ^2}}, as we know from newer results on the complexity of matrix inversion.
That is, we suppose \m {C_{\Rm {step}} = N_{H,t} ^2 N_{H,r} ^2}.
...
\DispNum {P:P:IN:H2} {
\NC \M {P} ^\Adj \M {P}
\approx \NC I _{N_{H,t} N_{H,r}} \NR
}
%
...
=\NC \MS {O} \SB {
   ...
   -\V {g}} _1} N_{H,t} ^2 N_{H,r} ^2 \NR
\NC =\NC \MS {O} \SB {\VNm {\M {P} ^\Adj \V {z}} _1} N_{H,t} ^2 N_{H,r} ^2 \NR
}
...
= \NC \s N_{H,t} N_{H,r} \MS {O} \RB {1} \NR
}
Then we simply have
%
\DispNum {C:S:ON:H6} {
\NC C_{\Rm {DS}}
=\NC \MS {O} \SB {N_{H,t} ^3 N_{H,r} ^3} \NR
}
...
\DispNum {C:P:ON:NY} {
\NC C_{\Rm {OMP}}
=\NC \MS {O} \SB {N_{H,t} N_{H,r} \log N_B} \NR
}
%
Here \m {\log N_B} is small in our experiments, and if we dropped it, we may take
\DispNum {C:P:ON:H2} {
\NC C_{\Rm {OMP}}
=\NC \MS {O} \SB {N_{H,t} N_{H,r}} \NR
}
...
However, Lasso and DS are equivalent in certain conditions \cite [AsR10], and we may suppose here they have the same complexity, \m {\MS {O} \SB {N_{H,t} ^3 N_{H,r} ^3}}.
...
Abbreviate \m {N_h = N_{H,t} \D N_{H,r}} as in the last chapter.

...
Candès and Tao \cite [Can05] showed that, in the noiseless case, a \m {\ell_1}-minimization program recovers the \m {N_m}-dimensional signal, with \m {N_p} measurements, under overwhelming probability.
...
Because the limit of hybrid structure, the designs which use a fully digital beamforming is not directly usable, and the designs which use only analog beamforming might not be optimal, which we have to investigate.
Moreover, we demonstrate that convex programming outperforms greedy methods, and may be necessary in less ideal situations.

In this treatise, we consider a hybrid structure with uniform linear array.
We generate random beamforming matrices, and use DS to estimate the channel in the space frequency domain.
In chapter 2 we shall see that our proposed sensing matrix has RIP for high probability, which may serve as the sensing matrix for DS.
In chapter 3, we give a quantitative bound (which holds for high probability) on expected error norm.
In chapter 4, numerical results show that in our problem configuration, DS is superior to other methods.
...
We claim that, for given number of sampling, DS recovers more successfully than OMP.
Moreover, our setting is more general than Alkhateeb, Leus, and Heath Jr.\ \cite [ALH15] with only analog combiner,
our random generation of sensing matrix is simpler than Lee, Gil, and Lee \cite [LGL16],
and we do not rely on additional assumption on spatially as in Gao, Dai, and Wang \cite [GDW15].

It is different from Bajwa et.\ al.\ \cite [BHS10], and while they require a series of random pilot vectors and the sensing stage take considerable time, we require only random sensing matrices and take just a few time frames.

In addition, we claim that with the same threshold, DS is better than Lasso.
Again, the proposed method is more general than Lian, Liu and Lau \cite [LLL17] with only analog combiner,

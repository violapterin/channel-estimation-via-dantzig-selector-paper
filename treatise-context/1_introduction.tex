...
Some instead applied least absolute shrinkage and selection operator (Lasso).
Indeed, it can be shown that Lasso and DS have similar behavior \cite [AsR10].
...
Gao, Dai, and Wang proposed a variant which we have the assumption of spatially common sparsity \cite [GDW15].
...
Do greedy algorithms trade precision for complexity, and is the opposite true of convex programs?
In this treatise, we consider a hybrid structure with uniform linear array as in Lee, Gil, and Lee \cite [LGL16].
...
Still, it is not clear whether the approximated scheme is optimal.

We illustrate that, with randomly generated beamformers, DS estimates the channel with better accuracy than other methods.
The setting is more general than \cite [LLL17] and more simple than \cite [VAT19], and the beamformer generation less complicated than \cite [DJN15].
...
Since DS is more accurate than the least square, OMP, and Lasso, it may be used when precision is crucial, possibly in combination of these lower-complexity methods.
It shall be useful when the ratio of the antenna and RF chain is larger, or when there does not allow many stage of estimation, that is, when the observable data is less sufficient.

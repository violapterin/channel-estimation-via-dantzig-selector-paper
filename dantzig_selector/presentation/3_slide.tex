...
{
\I For fixed \m {s =0, \dots N_p -1}, we say that \m {\M{\Phi}} satisfies the restricted isometry property (RIP) of sparsity \m {s} with respect to \m {0 \leq \d_s \leq 1}, if, for all \m {\V {x}}, and for all index set \m {\MC {A}} with \m {\# \MC {A} \leq s},
\Disp {
...
1. Approximating sum of largest \m {S} by integral
...
3. Assuming \m {\a_l} to be i.i.d.\ Gaussian
}
% % % % % % % % % % % % % % % % % % % % % % % % % %
...
{
\I To make expressions more compact, introduce the indication function \m {\i}, so that, for some injective \m {\s:\; \CB {0, \ldots, M-1} \CB {0, \ldots, N-1}},
\Disp {
\NC \NC \i_{a_0, \ldots, a_{M-1}} \SB {x_0, \ldots, x_{N-1}} \NR
\NC =\NC \startcases
...
\I For example \m {\i_{2} \SB {7,7} =1}, and \m {\i_{2,1,1} \SB {0,3,5,3} =1}
}
% % % % % % % % % % % % % % % % % % % % % % % % % %

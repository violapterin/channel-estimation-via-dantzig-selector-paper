...
Now, respectively: by Minkowski inequality; by H\"older's inequality; by the fact that \m {\M{F}_R} and \m {\M{F}_R} entries both have unity magnitude, by the identical distribution and by the definition of \m {M_{B,k}}; by Cauchy inequality: we have
...
\NC \leq \NC \Bigg(
  ...
  \sum_{l_H, l_H', m_H', l_H'' =0}^{N_H-1} \NR
    \NC \NC \SixQ
    \Big(
      \MB{E} \SB {\Nm {\M{F}_B ^{\Adj} \DB {n_R, l_Y}} ^{4k}} ^{1/4}
      \MB{E} \SB {\Nm {\M{F}_B ^{\Tr} \DB {n_R', l_Y}} ^{4k}} ^{1/4} \NR
      \NC \NC \SixQ
      \MB{E} \SB {\Nm {\M{W}_B ^{\ast} \DB {n_R, m_Y}} ^{4k}} ^{1/4}
      \MB{E} \SB {\Nm {\M{W}_B \DB {n_R', m_Y}} ^{4k}} ^{1/4}
    \Big) ^{1/k} \NR
  \NC \NC \SixQ
...
\NC \leq \NC \Gamma \SB {\F{k}{2} +1} ^4 \VNm {\V{u}} _2 ^{2k} \NR
}
...
Since the quantity \m {\Nm {\rho -\MB{E} \SB {\rho}}} is linear in \m {\VNm {\V{u}} _2}, by a scaling argument we may set, without loss of generality,
\Disp {
\NC \VNm {\V{u}} _2
=\NC 1 \NR[+]
}

...
%Now, respectively: by Minkowski inequality; by H\"older's inequality; by the fact that \m {\M{F}_R} and \m {\M{F}_R} entries both have unity magnitude, by the identical distribution and by the definition of \m {M_{B,k}}; by Power Mean inequality: we have
...
\bigg|
  ...
  \NC \NC \FourQ \V{u} \DB{l_H N_H +m_H}
...
\NC =\NC \MB{E} \Bigg[
  \Bigg|
    \sum_{l_H, l_H', m_H, m_H' =0}^{N_H-1}
      ...
      \NC \NC \FourQ \sum_{l_Y =0}^{N_Y-1} \RB{
        \sum_{n_R =0}^{N_R-1} \RB{
          \M{F}_B ^{\Adj} \DB {l_Y, n_R}
          \M{F}_R ^{\Adj} \DB {n_R, l_H}
        }
      ...
      \NC \NC \FourQ \sum_{m_Y =0}^{N_Y-1} \RB{
        \sum_{n_R =0}^{N_R-1} \RB{
          \M{W}_B ^{\ast} \DB {m_Y, n_R}
...
%
By Jensen's Inequality; by Power Mean Inequality to replace \m {l_H} and \m {l_H'} by the same number (resp.\ \m {m_H} and \m {m_H'}); by i.i.d.\ of \m {\M{F}_B} and \m {\M{W}_B} (resp.\ \m {\M{F}_R} and \m {\M{W}_R})
\Disp{
\NC \leq \NC \MB{E} \Bigg[
  \Nm{
    \sum_{l_H, l_H', m_H, m_H' =0}^{N_H-1}
  ...
  \NC \NC \Q \Q \sum_{l_H, l_H', m_H, m_H' =0}^{N_H-1}
    \V{u} \DB{l_H N_H +m_H}
    \V{u} ^\ast \DB{l_H' N_H +m_H'} \NR
    \NC \NC \FourQ \Bigg|
      \sum_{l_Y =0}^{N_Y-1}
...
By Cauchy Inequality and by and by assumption that \m {\V {u}} is unit norm; by i.i.d.\ of each quantity with \m {n_Y}; by spliting real and imaginary parts; by Jensen Inequality (or Power Mean Inequality),
\Disp{
...
%    \NC \NC \SixQ
%    \Big(
%      \MB{E} \SB {\Nm {\M{F}_B ^{\Adj} \DB {l_Y, n_R}} ^{4k}} ^{1/4}
%      \MB{E} \SB {\Nm {\M{F}_B ^{\Tr} \DB {l_Y, n_R''}} ^{4k}} ^{1/4} \NR
%      \NC \NC \SixQ
%      \MB{E} \SB {\Nm {\M{W}_B ^{\ast} \DB {m_Y, n_R'}} ^{4k}} ^{1/4}
%      \MB{E} \SB {\Nm {\M{W}_B \DB {m_Y, n_R'''}} ^{4k}} ^{1/4}
%    \Big) ^{1/k} \NR
%  \NC \NC \SixQ
...
%    \V{u} \DB{n_h} \V{u} ^\ast \DB{n_h'}
%  } ^k \NR
%\NC \leq \NC \Gamma \SB {2k +1} \VNm {\V{u}} _2 ^{2k} \NR
...
+\sum_{k=5}^\infty \F{1} {k!}
  \Gamma \SB {\F{k}{4} +1} ^4 s_{+} ^{k/2} \NR[+]
}

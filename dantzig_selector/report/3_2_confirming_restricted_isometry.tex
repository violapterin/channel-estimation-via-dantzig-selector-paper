...
In fact, it seems to be possible, from later analysis, to require that \m {\M{F}_R \DB {n_H, n_R}} be uniformly distributed only on the discrete set of \m {p}-th roots of unity if \m {p} is big enough.
...
\NC \leq \NC \RB {1 +N_Y^{-1}} \VNm {\V{u}} _2 ^4 \NR[+]
}
In simplifying we used \m {N_R \gg 1} and \m {N_Y^2 \gg 1}.
The decision to drop \m {\VNm {\V{u}} _4} term is justified by the fact \m {\VNm {\V{u}} _4 \geq N_H^{-1/2} \VNm {\V{u}} _2} and yet \m {N_H \gg N_Y}.

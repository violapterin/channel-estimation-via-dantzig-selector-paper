...
+\sum_{k=5}^\infty \F{1} {k!} \MB{E} \SB {s_+ ^k \rho ^{k/2}} \NR[+]
}
...
\NC \leq \NC 1
+s_+ \D \R {\MB{E} \SB {\rho}}
+\F{1} {2} s_+ ^2 \D \MB{E} \SB {\rho}
+\F{1} {6} \RB {1 +N_Y^{-2}} ^{3/4} s_+ ^3 \D \MB{E} \SB {\rho} ^{3/2} \NR
\NC \NC \FourQ +\F{1} {24} \RB {1 +N_Y^{-2}} s_+ ^4 \D \MB{E} \SB {\rho} ^{2}
+\sum_{k=5}^\infty
  \F{1} {k!} \G \SB {k +1} s_+ ^k
  \D \MB{E} \SB {\rho} ^{k/2} \NR
%
\NC \leq \NC 1
+s_+
+\F{1} {2} s_+ ^2
+\F{1} {6} \RB {1 +N_Y^{-2}} ^{3/4} s_+ ^3
+\F{1} {24} \RB {1 +N_Y^{-2}} s_+ ^4
...
If \m {s \ll 1}, we drop the last term (of \m {\MC {O} \SB {s ^5}}).
If \m {\e \ll 1} too, then
\Disp{
\NC \R {1+\e}
\geq \NC 1 +\F{1}{2} \e -\F{1}{8} \e^2 \NR
}
And recall that for \m {x \geq 0},
\Disp{
\NC \log \RB{1+x}
...
+\RB{s_+ +\F{1} {2} s_+ ^2 +\dots} \NR
\NC \NC \SixQ -\F{1}{2} \RB{s_+ +\F{1} {2} s_+ ^2 +\dots}^2
+\F{1}{3} \RB{s_+ +\F{1} {2} s_+ ^2 +\dots}^3 \NR
%
\NC \leq \NC \Min {s_+ >0}
...
Notice it is reasonable to assume \m {\e \leq 1}, so the square root is real.
If we furthermore keep only the leading term \m {\e ^{3/2}}, we get a simpler expression
\Disp {
\NC m
\leq \NC -\F{1}{4} N_Y \e ^{3/2} \NR[+]
}

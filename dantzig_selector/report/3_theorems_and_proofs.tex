...
\NC \NC \Q \Q +2 \Ss {i} \MB{E} \SB {\MF {Re} \SB {\M{F}_B \DB {n_R, n_Y}}}
...
{
\Disp {
\NC \NC \MB{E} \SB {\M{P} \DB {n_y, n_h} \M{P} \DB {n_y', n_h'}} \NR
\NC = \NC \startcases
\l_R^4 \l_B^4, \Q \MC n_y =n_y', n_h =n_h' \NR
...
where, restating (), \m {\M {Q} :=\RB {\M {F}_R^\Tr \M {F}_B^\Tr} \otimes \RB {\M {W}_B \M {W}_R}}.
...
\NC \i_2 \SB {x_0, x_1}
=\NC \startcases
\NC 1, \MC \Q x_0 =x_1 \NR
\NC 0, \NC \Q \Rm {otherwise} \NR
\stopcases \NR
\NC \i_4 \SB {x_0, x_1, x_2, x_3}
=\NC \startcases
\NC 1, \MC \Q x_0 =x_1 =x_2 =x_3 \NR
\NC 0, \NC \Q \Rm {otherwise} \NR
\stopcases \NR
}
And, for any permutation \m {\s} on \m {\CB {1, 2, 3, 4}},
\Disp {
\NC \i_{2,2} \SB {x_0, x_1, x_2, x_3}
=\NC \startcases
\NC 1, \MC \Q x_{\s \RB{0}} =x_{\s \RB{1}} \neq x_{\s \RB{2}} =x_{\s \RB{3}} \NR
\NC 0, \NC \Q \Rm {otherwise} \NR
\stopcases \NR
}
...
{
\Disp {
\NC \NC \MB{E} \SB {\RB {\M{F}_R \M{F}_B} \DB {n_H, n_Y} \RB {\M{F}_R \M{F}_B} \DB {n_H', n_Y'}} \NR
\NC = \NC \MB{E} \SB {\RB {\M{W}_R \M{W}_B} \DB {n_Y, n_H} \RB {\M{W}_R \M{W}_B} \DB {n_Y', n_H'}} \NR
\NC = \NC N_R \D \i_2 \SB {n_H, n_H'} \D \i_2 \SB {n_Y, n_Y'} \D \RB {\l_R^2 +\l_B^2} \NR
}
}

To see this, expanding the definition
\Disp {
\NC \NC \RB {\M{W}_B ^\Adj \M{W}_R ^\Adj} \DB {n_Y, n_H} \RB {\M{F}_R \M{F}_B} \DB {n_Y', n_H'} \NR
\NC =\NC
\RB {\sum_{n_R=0}^{N_R-1} \M{F}_R \DB {n_H, n_R} \M{F}_B \DB {n_R, n_Y}}
\RB {\sum_{n_R=0}^{N_R-1} \M{F}_R \DB {n_H', n_R} \M{F}_B \DB {n_R, n_Y'}} \NR
}
When either \m {n_y \neq n_y'} or \m {n_h \neq n_h'}, no term is an all-square product.
After taking expectation, the expectation sign is distributed, and every term vanishes.
But when \m {n_y =n_y'} or \m {n_h =n_h'}, the only terms that remains are
\Disp {
\NC \NC \sum_{n_R=0}^{N_R-1}
\MB{E} \SB {\M{F}_R \DB {n_H, n_R}^2}
\MB{E} \SB {\M{F}_B \DB {n_R, n_Y}^2} \NR
\NC = \NC N_R \D \RB {M_{R,2} +M_{B,2}} \NR
}

Having this in mind, a moment's reflection shows
\Result
{Lemma}
{
\Disp {
\NC \NC \MB{E} \SB {\M{Q} \DB {n_Y, n_H} \M{Q} \DB {n_Y', n_H'}} \NR
\NC = \NC \startcases
N_R \l_R^2 \l_B^2, \Q \MC n_Y =n_Y', n_H =n_H' \NR
0, \Q \NC \Rm {otherwise} \NR
\stopcases \NR
}
}

Fix any test vector \m {\V{u} \in \MB{V}_\MB{C} \SB{N_Y}}, we shall study the probability concentration of \m {\VNm {\M{F}_B \V{u}}_2^2} and \m {\VNm {\M{F}_B \V{u}}_2^4}.
...
\Nm {\M{Q} \DB {n_y, n_h}} ^4
\Nm {\V{u} \DB{n_h}} ^4 \NR
\NC =\NC N_Y^2 \D 4 N_R ^2 N_Y ^{-4} N_R ^{-4} \D \Nm {\V{u}} _4 ^4 \NR
\NC \leq \NC 4 N_Y ^{-2} N_R ^{-2} \Nm {\V{u}} _2 ^4 \NR[+]
}
\Disp {
\NC E_2
...
\Nm {\M{Q} \DB {n_y', n_h}} ^2
\Nm {\V{u} \DB{n_h}} ^4 \NR
\NC = \NC N_Y \D \F{1}{2} N_Y \RB{N_Y-1} \D 8 N_R ^{-2} N_Y ^{-4} \VNm {\V{u}} _2 ^4 \NR
\NC \NC \Q \Q + \F{1}{2} N_Y \RB{N_Y-1} \D N_Y^2 \D 4 N_R ^2 N_Y ^{-4} N_R ^{-4} \VNm {\V{u}} _2 ^4 \NR
\NC \leq \NC 4 N_R ^{-2} \VNm {\V{u}} _2 ^4 \NR[+]
}
\Disp {
\NC E_3
...
\Nm {\V{u} \DB{n_h}} ^2
\Nm {\V{u} \DB{n_h'}} ^2 \NR
\NC \leq \NC N_Y^2 \D \F{1}{2} N_H \RB{N_H-1} \D 8 N_R ^{-2} N_Y ^{-4} \RB {\VNm {\V{u}} _2 ^4 -\VNm {\V{u}} _4 ^4} \NR
\NC \leq \NC 4 N_R ^{-2} N_Y ^{-1} \VNm {\V{u}} _2 ^4 \NR[+]
}
\Disp {
\NC E_4
...
\Nm {\V{u} \DB{n_h}} ^2
\Nm {\V{u} \DB{n_h'}} ^2 \NR
\NC \leq \NC N_Y \D \F{1}{2} N_H \RB{N_H-1} \D 8 N_R ^{-2} N_Y ^{-4} \RB {\VNm {\V{u}} _2 ^4 -\VNm {\V{u}} _4 ^4} \NR
\NC \leq \NC 4 N_H ^2 N_R ^{-2} N_Y ^{-3} \VNm {\V{u}} _2 ^4 \NR[+]
}
The last one is more difficult.
In the end we used Muirhead inequality to get \m {\i_{1,1,1,1} \SB {n_h, n_h', m_h, m_h'} \V{u} \DB{n_h} \V{u} \DB{n_h'} \V{u} \DB{m_h} \V{u} \DB{m_h'} \leq \RB {1/6} \RB {\VNm {\V{u}} _2 ^4 -\VNm {\V{u}} _4 ^4}}.
\Disp {
\NC E_5
\NC = \sum _{
    \Stack {
  ...
  \Nm {\M{Q} \DB {n_y, n_h'}}
  \Nm {\M{Q} \DB {n_y, m_h}}
  \Nm {\M{Q} \DB {n_y, m_h'}} \NR
  \NC \NC \Q \Q \Q \Q
  \VNm {\V{u} \DB{n_h}} ^2
  \VNm {\V{u} \DB{n_h'}} ^2
  \VNm {\V{u} \DB{m_h}} ^2
  \VNm {\V{u} \DB{m_h'}} ^2 \NR
\NC =\NC N_Y \D \F{1}{2} N_H \RB{N_H-1} \D \F{1}{2} N_Y \RB{N_Y-1} \D N_H^2 \NR
\NC \NC \Q \Q \Q \Q
\D 8 N_R ^{-2} N_Y ^{-4} \NR
\NC \NC \Q \Q \Q \Q
\D \sum_{\Stack {n_h, n_h' =0}} ^{N_H-1}
\sum_{\Stack {m_h, m_h' =0}} ^{N_H-1}
\i_{1,1,1,1} \SB {n_h, n_h', m_h, m_h'} \NR
\NC \NC \Q \Q \Q \Q
\VNm {\V{u} \DB{n_h}} \VNm {\V{u} \DB{n_h'}}
\VNm {\V{u} \DB{m_h}} \VNm {\V{u} \DB{m_h'}} \NR
\NC \leq \NC \F{1}{3} N_Y ^{-1} N_H ^4 N_R ^{-2} \VNm {\V{u}} _2 ^4. \NR[+]
}

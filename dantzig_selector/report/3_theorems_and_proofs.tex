...
\leq \NC 4 N_H^2 N_Y^{-3} \VNm {\V{u}} _2 ^4 \NR[+]
}
...
\Disp {
\NC \VNm {\V {d}} _2
\leq \NC \F {4}{\pi^4} \d_{3S}^4 \RB {1-2\d_{2S}} S^4 \RB {\log N_H}^4 \NR
}
}
...
The probability that it is true may be bounded by the number in Lemma () and Lemma ().
Bounding generously with \m {\d_s \leq 1} yields
\Disp {
\NC 1 -p
\leq \NC 4 \D 12^S N_H^2 N_Y^{-3} +N_H^{-2} \NR[+]
}
If \m {N_Y \leq N_H}, there is a condition imposed on the size of \m {N_H}.
Arranging gives
\Disp {
\NC \log N_H
\geq \NC s^2 L \log 12 + \log 4 \NR[+]
}

...
Start with the density of each entry of \m {\M{F}_B}, namely i.i.d.\ Gaussian r.v.\ with mean 0, standard deviation \m {1/2}.
They are (with a slight of abuse of meaning of \m {\o_t} and so on)
\Disp{
\NC d \o_t
=\NC \F {1} {\R {\pi}} \exp
  \RB {-\MF{Re} \SB {\M{F}_B \DB {n_R, n_Y} \SB {\o_t}} ^2}
d \MF{Re} \SB {\M{F}_B \DB {n_R, n_Y}} \NR[+]
\NC d \o_t
=\NC \F {1} {\R {\pi}} \exp
  \RB {-\MF{Im} \SB {\M{F}_B \DB {n_R, n_Y} \SB {\o_t}} ^2}
...
\Disp{
\NC \MB{E} _{\o_t} \SB {\M{F}_B \DB {n_R, n_Y} \SB {\o_t}}
=\NC 0 \NR
\NC \MB{E} _{\o_t} \SB {\Nm {\M{F}_B \DB {n_R, n_Y} \SB {\o_t}}^2}
=\NC 1 -\F {\pi}{4} \NR
}
Fix test vector \m {\V{u} \in \MB{M}_\MB{C} \SB{N_Y}}, we have to study \m {\VNm {\M{F}_B \V{u}}_2}.
\Disp{
\NC \NC \MB{E} _{\o_t} \SB {\VNm {\M{F}_B \V{u}} _2 ^2} \NR
\NC = \NC \MB{E}
\SB {
  \sum_{n_R=0}^{N_R-1}
...
\NC = \NC \sum_{n_R=0}^{N_R-1}
  \MB{E} _{\o_t} \SB {\Nm {\sum_{n_Y=0}^{N_Y-1} \M{F}_B \DB {n_R, n_Y} ^* \V{u} \DB{n_Y}}^2}
\NR
\NC = \NC
\sum_{n_R=0}^{N_R-1}
  \MB{E} _{\o_t} \SB {
      2 \sum_{n_Y, n_Y'=0}^{N_Y-1}
         \MF {Re} \SB{
...
2 \sum_{n_Y, n_Y'=0}^{N_Y-1}
\MF {Re} \SB{
   \MB{E} _{\o_t} \SB {\M{F}_B \DB {n_R, n_Y}}
   \MB{E} _{\o_t} \SB {\M{F}_B \DB {n_R, n_Y'}}
}
\V{u} \DB{n_Y} ^* \V{u} \DB{n_Y'} ^*
\NR
\NC \NC \quad \quad +\sum_{n_R=0}^{N_R-1}
   \sum_{n_Y=0}^{N_Y-1} \MB{E} _{\o_t} \SB {\Nm {\M{F}_B \DB {n_R, n_Y}}^2} \Nm {\V{u} \DB{n_Y}}^2
\NR
\NC = \NC \RB {1 -\F{\pi}{4}} N_Y N_R \VNm {\V{u}} _2 ^2 \NR[+]
}

Thus, by Chebyshev bound,
\Disp{
\NC \MB{P} _{\o_t} \SB {
   \Nm {\VNm {\M{F}_B \V{u}} _2 -\VNm {\V{u}} _2}
   \geq \e \VNm {\V{u}}
}
\leq \NC \RB {1 -\F{\pi}{4}} \F {N_Y N_R} {\e} \NR
}

...
Denote the sorting function to be \m {f:\; T\RB {N} \mapsto \MB {R}_+}, \m {N \in \MB {N}}.
That is, for \m {x \in \MB {V}_{\MB {K}} \SB {N}}, \m {x} is confined in sorted magnitude according to \m {f}.
...
\Disp {
\NC \sum_{n=s}^N \Nm {\MC{S} \SB {\V {x}} \SB {n}}
\leq \NC R. \NR
}
}
...
Then \m {\V {a} \SB {\f}} is almost-\m {L}-sparse according to the \m {\ell_2}-norm with remainder \m {R} to be
\Disp {
\NC R
= \NC \F {1}{\pi} \log \F {N_H}{L-1}. \NR
}
}

We now show the lemma, which is the key to bounding the sparsity of \m {h}.
Let \m {\f'} be fixed.
What we have to bound is
\Disp {
\NC R \SB {\f'}
=\NC \sum_{n_H =s}^{N_H -1}
   \RB {\MC{S} \SB {\M {K}^\Adj \V {a} \SB {\f}}} \DB {n_H} \NR[+]
}
Observe
\Disp {
\NC \RB {\M {K}^\Adj \V {a} \SB {\f}} \DB {n_H}
=\NC \F {1}{N_H} D \SB {\RB {\f \; \Rm{mod}\; \F{2\pi}{N_H}} +\F {2 \pi n_H} {N_H}} \NR
}
where \m {D} is the Dirichlet kernel
\Disp {
\NC D \SB {\f'}
:= \NC \Nm { \sum_{n_H=0}^{N_H-1} \Rm {e}^{i n_H \f'} } \NR
...
\NC \Nm {D \SB {\f'} }
\leq \NC B \SB {\f'} \NR
\NC := \NC \F {48}{\Nm {\f'^2 -24} \Nm {\f'}} \NR[+]
\NC -\pi \leq \NC \f' < \pi. \NR
}
Thus,
\Disp {
\NC R \SB {\f'}
=\NC \sum_{n_H =s}^{N_H -1}
   ...
   \SB {
      \sum_{n_H' =s}^{N_H -1}
      B \SB {\RB {\f \; \Rm{mod}\; \F{2\pi}{N_H}} +\F {2 \pi n_H'} {N_H}}
      \V {u}_{n_H'}
   }
   \DB {n_H}
\NR[+]
}
Notice that \m {\Nm {B \SB {\phi'}}} is monotone, and then
\Disp{
\NC \NC \MC{S}
\SB {
   \sum_{n_H' =s}^{N_H' -1}
...
\DB {n_H} \NR
\NC =\NC B \SB {\RB {\f \; \Rm{mod}\; \F{2\pi}{N_H}} +\F {2 \pi n_H} {N_H}} \NR
\NC \leq \NC B \SB {\F {2 \pi n_H} {N_H}} \NR[+]
}
What we saw gives
\Disp{
\NC R \SB {\f'}
=\NC \F {1}{N_H} \sum_{n_H =s}^{N_H -1}
   B \SB {\F {2 \pi n_H} {N_H}} \NR
}

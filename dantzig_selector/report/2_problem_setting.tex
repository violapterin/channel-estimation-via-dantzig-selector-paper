...
\NC \V {a} \SB {\psi'}
= \NC \F {1}{\R {N_H}} \sum_{n=0}^{N-1} \Mr {e}^{n \psi' \Mr {i}} \V {u}_n \NR
}
where \m {\Mr {e}} is the base of natural logarithm, and \m {\Mr {i}} the imaginary unit.

And consider the virtual representation of the MIMO channel (see for example Akdeniz et.\ al.\ 2014).
\Disp {
\NC \M{H}
=\sum_{l=1}^L \alpha_l \V {a} \SB { \F {d_{\Mr {arr}}} {\lambda} \sin \f_l' } \V {a} \SB { \F {d_{\Mr {arr}}} {\lambda} \sin \th_l' }^\Adj \NR
}
...
\Disp {
\NC \f_l
= \NC \F {d_{\Mr {arr}}} {\lambda} \sin \f_l' \quad \rm{Mod}\; (2\pi) \NR[+]
\NC \th_l
= \NC \F {d_{\Mr {arr}}} {\lambda} \sin \th_l' \quad \rm{Mod}\; (2\pi) \NR[+]
}
...
\in \NC \Mb {V}_{\Mb {C}} \SB {N_y} \NR
\NC \V {z}
:= \NC \Mr {vec} \SB {\M{W}_B \M{W}_R \M {Z}} \NR[+]
\NC \M {P}^\star
...
\NC \M {K} \DB {n_1, n_2} = \NC \F {1} {\R {N_H}} \Mr {e}^{2\pi \Mr{i} n_1 n_2 /N_H}, \NR[+]
\NC \quad n_1, n_2 = \NC 0, 1, \dots, N_H-1 \NR
}
Roman \m{\Mr{e}} and \m{\Mr{i}} denote the natural base and imaginary unit, respectively.
...
\NC \Mr {subject} \; \Mr {to} \quad  \MC \VNm {\M {P}^\Adj \RB {\V {y} -\M {P} \V {g}'}} _\infty \leq \g \NR
\stopcases \NR
}

...
where \m {L} is the number of paths, \m {\f_l} the angle of arrival, \m {\th_l} the angle of arrival
}
% % % % % % % % % % % % % % % % % % % % % % % % % %
...
\I Rx: Digital combiner \m {\M {W}_R \in \NC \MB {C} ^{N_R \D N_H}}

\I Rx: Analog combiner \m {\M {W}_B \in \NC \MB {C} ^{N_Y \D N_R}}

\I With the unity magnitude constraint
\Disp {
\NC \Nm {\M {F}_R \SB {n_H, n_R}}
= \NC 1, \NR
\NC \Nm {\M {W}_R \SB {n_R, n_H}}
= \NC 1, \NR
}

\I Under the assumption \m {N_H \gg N_R \gg N_Y}
}
% % % % % % % % % % % % % % % % % % % % % % % % % %
\Frame {Effective Channel}
{
\I Consider zero-mean, unity-variance additive white Gaussian noise (AWGN) \m {\M{Z} \in \NC \MB {C} ^{N_H \D N_H}}
...
\I How do we recover \m {\M {H}} with knowledge of \m {\M {Y}}?

\I How do we design \m {\M {W}_R}, \m {\M {W}_B}, \m {\M {F}_R}, and \m {\M {F}_B}?
}
% % % % % % % % % % % % % % % % % % % % % % % % % %
...
\NC \V {g} := \NC \Rm {vec} \SB {\M {K}^\Adj \M {H} \M {K}} \NR
...
\I Recover \m {\Hat {\M {H}} \LA \M {K} \Rm {vec}^{-1} \SB {\Hat {g}} \M {K}^\Adj}

...
\I \Emph{Q4}: Cand\`es and Tao showed the Big-O error bound in terms of (in our case) \m {N_H}, but does, for practical values, the algorithm converges for \Emph {high probability}?
}
% % % % % % % % % % % % % % % % % % % % % % % % % %
...
\I Take into consideration how not exactly sparse \m {\V{g}} is.

\I Find \m {\MB {E} \SB {\rho}} and \m {\MB {E} \SB {\rho^2}}, and bound \m {\MB {E} \SB {\rho ^k}} for \m {k=3,4,5,\dots}.

\I Bound the probability that \m {\Nm {\rho -1} \geq \e}.

\I Establish that \m {\M{P}} satisfies RIP for certain probability.
}
% % % % % % % % % % % % % % % % % % % % % % % % % %
...
{
\I Call the vector of largest-magnitude \m {N_H L} components of \m {\V{g}} to be \m {\V{g}_A}, and remaining components of \m {\V{g}} to be \m {\V{g}_K}, so that \m {\V{g}_A +\V{g}_K =\V{g}}.
...
\I Note that \m {R \to 0} as \m {N_H \to \infty}
}
% % % % % % % % % % % % % % % % % % % % % % % % % %
...
{
\I Considering \Emph {Q2}: If each entry of \m {\M {P}} is i.i.d., then it's likely that, by whatever variation of \Emph {Central Limit Theorem},
\Disp {
\NC \MB {E} \SB {\rho ^{k}}
\approx \NC 1
...
\I Try to set each entry of \m {\M{F}_B} (resp.\ \m {\M{W}_B}) to be i.i.d.\ Complex Standard Normal, multiplied by a design constant \m {\l_B >0} (so that the magnitude follows Rayleigh distribution).

\I And let each entry of \m {\M{F}_R} (resp.\ \m {\M{W}_R}) be uniformly distributed on the unit circle on the complex plane, multiplied by a design constant \m {\l_B >0}.
}
% % % % % % % % % % % % % % % % % % % % % % % % % %
\Frame {Chernoff Inequality}
{
\I Imitating the \Emph {Chernoff Inequality} argument in classical probability, we have, for any \m {s >0} so that, supposedly, both sides \m {< \infty},
\Disp {
\NC p
...
{
\I We can show that, by tedius calculation,
\Disp {
\NC \MB{E} \SB {\rho ^2}
...
{
\I We can show that, with applications of various inequalities,
\Disp {
\NC \MB{E} \SB {\rho ^k}
...
\Disp {
\NC \NC \log p \NR
\NC \leq \NC \Min {s >0} \SB {
  -\RB {1 +\e} s
  +\log \RB{
    1 +s \MB{E} \SB {\rho}
    +\F{s ^2} {2} \MB{E} \SB {\rho^2} +\dots
  }
} \NR
}
...
{
\I Take square roots on both sides so that the remainder term is finite for \m {s <1}.
\Disp {
\NC p
= \NC \MB{P} \SB {\rho \geq \RB {1 +\e} \MB{E} \SB {\rho}} \NR
...
\I The \m {1/2} and \m {3/2} moments can be bounded by Cauchy Ineq.:
\Disp{
\NC \F{1}{\R{1+N_Y^{-2}}}
...
% % % % % % % % % % % % % % % % % % % % % % % % % %
\Frame {Expected Error of Dantzig Selector}
{

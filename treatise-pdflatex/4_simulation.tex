...
It might have something to do with appropriately scaling \m {\g_{\mathrm {DS}}} with respect to \m {N_H}.
...
We take \m {\h_{\mathrm {OMP}} = 2 \R {\log N_H}} for \m {\infty}-norm condition, \m {\h_{\mathrm {OMP}} = \R {3} \SB {N_{B,r} N_{B,t}} ^ {1/4}} for 2-norm condition.
...
Third, when \m {N_{B,t} /N_{B,r}} is smaller, \m {\tilde {\chi} _{\mathrm {DS}}} is smaller, because we have a higher sample rate.
As \m {N_{B,t} /N_{B,r}} increase for given \m {\s}, \m {\tilde {\chi} _{\mathrm {DS}}} improves more significantly.
...
Suppose a linear program has \m {N} dimension variable and \m {M} inequality constraints, then the complexity is \m {\mathcal {O} \SB {N^2 M}}, assuming the Dantzig simplex method is used (\cite {BoV04}, p.6).
...
Simulation illustrates that \m {t_{\mathrm {DS}}} grows tremendously with \m {N_H}, agreeing the criticism by Friedlander and Saunders \cite {FrS07} on the high complexity of DS.

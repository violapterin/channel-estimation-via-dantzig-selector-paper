...
Remove the largest-magnitude \m {S} components of \m {\V{g}}, and call the remaining vector \m {\M {g} _{\SB{\MC {C}}}}, so that it remains to bound \m {\VNm {\M {g} _{\SB{\MC {C}}}}}.

\I Establish that \m {\M{W}_R} and \m {\M{W}_R} satisfies RIP, thus \m {\M{P}} satisfies RIP too, for high probability.
...
In fact, if \m {\M {G} =\M {U} \Rm {diag} \SB {\V {d}} \M {U} ^\Adj}, for some \m {S}-sparse \m {\V {d}} and unitary \m {\M {U}}.
...
%
\NC \eqsim \NC \VNm {\M {K} \M {H} \M {K} ^\Adj} _2 \RB {1 +2\d_s} \NR
}
}
% XXX % % XXX % % XXX % % XXX % % XXX % % XXX %
